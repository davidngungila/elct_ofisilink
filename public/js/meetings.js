...
$(document).ready(function() {
    ...
    function init() {
        ...
        initializeSelect2();
        // Update pending count badge after a short delay to ensure DOM is ready
        setTimeout(updatePendingCountBadge, 500);
    }

    // Setup Event Listeners
    function setupEventListeners() {
        // Navigation - Updated to use independent pages
        $('#create-meeting-btn').on('click', function(e) {
            e.preventDefault();
            window.location.href = '/modules/meetings/new';
        });
        $('#manage-categories-btn').on('click', function(e) {
            e.preventDefault();
            window.location.href = '/modules/meetings/categories';
        });
        $('#create-minutes-btn').on('click', function(e) {
            e.preventDefault();
            openMinutesModal();
        });
        $('#pending-approvals-btn').on('click', function(e) {
            e.preventDefault();
            window.location.href = '/modules/meetings/pending-approval';
        });
        ...
        // Meeting Actions - Navigate to independent pages
        $(document).on('click', '.view-meeting-btn', function() { 
            const id = $(this).data('id');
            window.location.href = `/modules/meetings/${id}`;
        });
        $(document).on('click', '.edit-meeting-btn', function() { 
            const id = $(this).data('id');
            window.location.href = `/modules/meetings/${id}/edit`;
        });
    ...
    function loadMeetings() {
        // Get branch_id from URL or filter
        const urlParams = new URLSearchParams(window.location.search);
        const branchId = $('#filter-branch').val() || urlParams.get('branch_id') || '';
        ...
        return `
            <tr>
                <td>
                    ...
                    ${meeting.has_minutes ? '<span class="badge bg-success ms-1"><i class="bx bx-file"></i> Minutes</span>' : ''}
                    ${meeting.branch_name ? `<br><small class="text-muted"><i class="bx bx-map"></i> ${escapeHtml(meeting.branch_name)}</small>` : ''}
                </td>
    ...
    function renderMeetingReview(meeting) {
                ...
                <div class="col-md-8">
                    <div class="review-section">
                        ...
                        <table class="table table-sm">
                            ...
                            ${meeting.branch_name ? `<tr><td><strong>Branch:</strong></td><td>${escapeHtml(meeting.branch_name)} ${meeting.branch_code ? '(' + escapeHtml(meeting.branch_code) + ')' : ''}</td></tr>` : ''}
                            ...
                            <tr><td><strong>Venue:</strong></td><td>${escapeHtml(meeting.venue || 'TBD')}</td></tr>
                            <tr><td><strong>Type:</strong></td><td>${meeting.meeting_type || 'physical'}</td></tr>
        ...
        const canApprove = meeting.status === 'pending_approval';
        if (canApprove) {
            $('#approve-meeting-btn, #reject-meeting-btn').show();
            $('#edit-meeting-btn').show(); // Allow editing pending meetings
        } else {
            $('#approve-meeting-btn, #reject-meeting-btn').hide();
            // Hide edit button for approved/completed meetings
            if (meeting.status === 'approved' || meeting.status === 'completed') {
                $('#edit-meeting-btn').hide();
            } else {
                $('#edit-meeting-btn').show();
            }
        }
    }
    ...
    function updatePendingCountBadge() {
        $.ajax({
            url: ajaxUrl,
            method: 'POST',
            data: { _token: csrfToken, action: 'get_meetings', status: 'pending_approval' },
            success: function(response) {
                if (response.success && response.meetings) {
                    const count = response.meetings.length;
                    $('#pending-count-badge, #header-pending-count').text(count);
                }
            }
        });
    }
