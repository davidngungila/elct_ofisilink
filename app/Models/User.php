...
// use Spatie\Permission\Traits\HasRoles; // Not using Spatie permission tables
...
class User extends Authenticatable
{
    ...
    /**
     * Get the branches this user manages
     */
    public function managedBranches()
    {
        return $this->belongsToMany(Branch::class, 'branch_managers', 'user_id', 'branch_id')
                    ->withTimestamps();
    }
    ...
    /**
     * Get all permissions for the user (alias for customPermissions)
     */
     ...
     * Checks permissions through user's roles using the custom permission system
     */
    public function hasPermission($permission)
    {
        ...
        foreach ($this->roles as $role) {
            if ($role->relationLoaded('permissions') && $role->hasPermission($permission)) {
                return true;
            }
        }
    ...
    {
        return $this->belongsToMany(TaskActivity::class, 'activity_assignments', 'user_id', 'activity_id')
