...
class PettyCashController extends Controller
{
    ...
    public function show(PettyCashVoucher $pettyCash)
    {
        ...
        // 3. Accountant verified at creation time (accountant_verified_at is set within 5 seconds of created_at)
        ...
        // CRITICAL: Must verify at creation time (within 5 seconds) to distinguish from regular vouchers verified later
        $isDirectVoucher = false;
        if ($pettyCash->created_by !== null &&
            ...
            // Check if verification happened at creation time (within 5 seconds)
            if ($pettyCash->created_at && $pettyCash->accountant_verified_at) {
                $timeDifference = abs($pettyCash->created_at->diffInSeconds($pettyCash->accountant_verified_at));
                $isDirectVoucher = $timeDifference <= 5;
            }
        }
    ...
    public function directVouchersIndex(Request $request)
    {
        ...
        // Helper to check if voucher is direct (created by accountant and verified immediately)
        // Direct vouchers are verified at creation (within 5 seconds)
        $isDirectVoucher = function($voucher) {
            if ($voucher->created_by === null || $voucher->accountant_id === null) {
                return false;
            }
            if ($voucher->created_by !== $voucher->accountant_id) {
                return false;
            }
            if ($voucher->accountant_verified_at === null) {
                return false;
            }
            // Critical: Verification must happen at creation (within 5 seconds)
            if ($voucher->created_at && $voucher->accountant_verified_at) {
                $timeDifference = abs($voucher->created_at->diffInSeconds($voucher->accountant_verified_at));
                return $timeDifference <= 5;
            }
            return false;
        };
        
        // Get counts for direct vouchers - must verify at creation time
        $directPendingHodCount = PettyCashVoucher::where('status', 'pending_hod')
            ->whereColumn('created_by', 'accountant_id')
            ->whereNotNull('accountant_id')
            ->whereNotNull('accountant_verified_at')
            ->whereRaw('ABS(TIMESTAMPDIFF(SECOND, created_at, accountant_verified_at)) <= 5')
            ->count();
        $directPaidCount = PettyCashVoucher::where('status', 'paid')
            ->whereColumn('created_by', 'accountant_id')
            ->whereNotNull('accountant_id')
        ...
        // Build query for direct vouchers only - must verify at creation time
    ...
    public function hodApprove(Request $request, PettyCashVoucher $pettyCash)
    {
        ...
        try {
            ...
            // CRITICAL: Check if verification happened at creation time (within 5 seconds)
            // This distinguishes direct vouchers (verified at creation) from regular vouchers (verified later)
            $isDirectVoucher = false;
            if ($pettyCash->created_by !== null &&
                $pettyCash->accountant_id !== null &&
                $pettyCash->created_by === $pettyCash->accountant_id &&
            ...
            \Log::info('HOD Approval - Voucher Detection', [
                ...
                'time_difference_seconds' => $pettyCash->created_at && $pettyCash->accountant_verified_at ? abs($pettyCash->created_at->diffInSeconds($pettyCash->accountant_verified_at)) : null,
    ...
    public function submitRetirement(Request $request, PettyCashVoucher $pettyCash)
    {
        ...
        // Check if this is a direct voucher - Direct vouchers cannot be retired
        // CRITICAL: Must check timestamp to ensure verification happened at creation
        $isDirectVoucher = false;
        if ($pettyCash->created_by !== null &&
            $pettyCash->accountant_id !== null &&
            $pettyCash->created_by === $pettyCash->accountant_id &&
        ...
        if ($isDirectVoucher) {
            return redirect()->back()->with('error', 'Direct vouchers (in-office expenses already used) cannot be retired. They are automatically complete after HOD approval.');
        }
