...
<div class="container-xxl flex-grow-1 container-p-y">
    ...
    @php
        ...
        $hasFilters = !empty($search) || !empty($branch) || !empty($department) || 
                      !empty($employmentType) || !empty($status) || !empty($salaryRange);
        
        $activeFilters = array_filter([$search, $branch, $department, $employmentType, $status, $salaryRange]);
            ...
            <div class="card">
                <div class="card-header">
                    <div class="d-flex justify-content-between align-items-center">
                        ...
                        <button class="btn btn-sm btn-outline-primary" type="button" data-bs-toggle="collapse" data-bs-target="#filterPanel" aria-expanded="{{ $hasFilters ? 'true' : 'false' }}" aria-controls="filterPanel">
                            <i class="bx {{ $hasFilters ? 'bx-chevron-up' : 'bx-chevron-down' }} me-1"></i>Toggle Filters
                        </button>
                    </div>
                </div>
                <div class="collapse @if($hasFilters) show @endif" id="filterPanel">
                    <div class="card-body">
                        <div class="row g-3">
                            <div class="col-md-2">
                            ...
                            <div class="col-md-2">
                                ...
                                <select class="form-select" id="branchFilter" onchange="filterByBranch(this.value)">
                            ...
                            <div class="col-md-2">
                                ...
                                <select class="form-select" id="salaryRangeFilter">
                                    ...
                                    <option value="0-500000" {{ request('salary_range') == '0-500000' ? 'selected' : '' }}>0 - 500K</option>
                                    <option value="500000-1000000" {{ request('salary_range') == '500000-1000000' ? 'selected' : '' }}>500K - 1M</option>
                                    <option value="1000000-2000000" {{ request('salary_range') == '1000000-2000000' ? 'selected' : '' }}>1M - 2M</option>
                                    <option value="2000000+" {{ request('salary_range') == '2000000+' ? 'selected' : '' }}>2M+</option>
                                </select>
                            </div>
                            <div class="col-md-1">
                                <label class="form-label">&nbsp;</label>
                                <button class="btn btn-primary w-100" onclick="applyFilters()" title="Apply Filters">
                                    <i class="bx bx-filter"></i>
                                </button>
                            </div>
                            <div class="col-md-1">
                                <label class="form-label">&nbsp;</label>
                                <button class="btn btn-outline-secondary w-100" onclick="clearAllFilters()" title="Clear All Filters">
                                    <i class="bx bx-x"></i>
                                </button>
                            </div>
                        </div>
    ...
    <!-- Branch Information Panel (when filtered by branch) -->
    @if(isset($selectedBranch) && $selectedBranch)
    <div class="row mb-4">
        <div class="col-12">
            <div class="card border-primary shadow-lg" style="border-left: 5px solid #007bff;">
                <div class="card-header bg-primary text-white">
                    <div class="d-flex justify-content-between align-items-center">
                        <div>
                            <h5 class="mb-0 text-white">
                                <i class="bx bx-map me-2"></i>{{ $selectedBranch->name }}
                                @if($selectedBranch->code)
                                    <small class="text-white-50">({{ $selectedBranch->code }})</small>
                                @endif
                            </h5>
                        ...
                        <div class="col-md-8">
                            <div class="row mb-3">
                                <div class="col-md-6">
                                    ...
                                    <p class="mb-0">{{ $selectedBranch->address ?? 'N/A' }}</p>
                                </div>
                                <div class="col-md-3">
                                    <strong><i class="bx bx-phone me-2 text-primary"></i>Phone:</strong>
                                    <p class="mb-0">{{ $selectedBranch->phone ?? 'N/A' }}</p>
                                </div>
                                <div class="col-md-3">
                                    <strong><i class="bx bx-envelope me-2 text-primary"></i>Email:</strong>
                                    <p class="mb-0">{{ $selectedBranch->email ?? 'N/A' }}</p>
                                </div>
                            </div>
                            @if($selectedBranch->managers && $selectedBranch->managers->count() > 0)
                            <div class="mb-3">
                                <strong><i class="bx bx-user-check me-2 text-primary"></i>Branch Managers:</strong>
                                <div class="mt-2">
                                    @foreach($selectedBranch->managers as $manager)
                                        <span class="badge bg-info me-2">{{ $manager->name }}</span>
                                    @endforeach
                                </div>
                            </div>
                            @endif
                        ...
                        <div class="col-md-4">
                            <div class="card bg-light">
                                <div class="card-body">
                                    ...
                                    <div class="d-flex justify-content-between mb-2">
                                        <span>Active:</span>
                                        <strong class="text-success">{{ $selectedBranch->users->where('is_active', true)->count() }}</strong>
                                    </div>
                                    <div class="d-flex justify-content-between mb-2">
                                        <span>Inactive:</span>
                                        <strong class="text-secondary">{{ $selectedBranch->users->where('is_active', false)->count() }}</strong>
                                    </div>
                                    <div class="d-flex justify-content-between">
                                        <span>Managers:</span>
                                        <strong class="text-info">{{ $selectedBranch->managers->count() }}</strong>
                                    </div>
                                </div>
                            </div>
                        </div>
...
function applyFilters() {
    const searchTerm = $('#advancedSearch').val();
    ...
    // Set or remove department parameter
    if (department) {
    ...
    // Set or remove employment_type parameter
    if (employmentType) {
    ...
    // Set or remove salary_range parameter
    if (salaryRange) {
        url.searchParams.set('salary_range', salaryRange);
    } else {
        url.searchParams.delete('salary_range');
    }
    
    // Set or remove branch parameter
    if (branch) {
...
// Filter by Branch (standalone function for onchange)
...
$(document).ready(function() {
    ...
    @if($hasFilters)
        $(document).ready(function() {
            ...
            if (filterPanel.length && !filterPanel.hasClass('show')) {
                filterPanel.addClass('show');
                // Update button state
                $('[data-bs-target="#filterPanel"]').attr('aria-expanded', 'true');
                $('[data-bs-target="#filterPanel"] i').removeClass('bx-chevron-down').addClass('bx-chevron-up');
            }
        });
    @endif
    
    // Update toggle button icon when collapse state changes
    $('#filterPanel').on('show.bs.collapse', function () {
        $('[data-bs-target="#filterPanel"] i').removeClass('bx-chevron-down').addClass('bx-chevron-up');
    });
    
    $('#filterPanel').on('hide.bs.collapse', function () {
        $('[data-bs-target="#filterPanel"] i').removeClass('bx-chevron-up').addClass('bx-chevron-down');
    });
});
</script>
@endpush