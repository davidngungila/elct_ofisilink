...
class PhysicalRackController extends Controller
{
    ...
    public function dashboard(Request $request)
    {
        ...
        // Get root rack folders (no parent structure for physical racks) - filter by branch
        $rootFoldersQuery = RackFolder::with(['category', 'department', 'creator', 'branch'])
        ...
        if ($selectedBranchId && !$canViewAll) {
            $rootFoldersQuery->where('branch_id', $selectedBranchId);
            ...
            $rootFoldersQuery->where('branch_id', $userBranchId);
        }
        ...
        // Get dashboard stats (filtered by branch)
        $stats = $this->getDashboardStats($user, $canViewAll, $currentDeptId, $selectedBranchId);
        
        // Get recent activities (filtered by branch)
        $recentActivityQuery = RackActivity::with(['user', 'folder'])
            ->whereHas('folder', function($q) use ($selectedBranchId, $userBranchId, $canViewAll) {
                if ($selectedBranchId) {
    ...
    private function getDashboardStats($user, $canViewAll, $currentDeptId, $branchId = null)
    {
        ...
        $foldersQuery = RackFolder::query();
        $filesQuery = RackFile::query();
    ...
    private function createRackFolder(Request $request)
    {
        ...
        // Get branch_id from user's branch
    ...
    private function createRackFile(Request $request)
    {
        ...
        $folder = RackFolder::findOrFail($validated['folder_id']);
