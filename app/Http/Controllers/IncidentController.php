...
class IncidentController extends Controller
{
    ...
    public function create()
    {
        ...
        if (!$user->hasAnyRole(['HR Officer', 'HOD', 'System Admin', 'Reception Person'])) {
            abort(403, 'You do not have permission to create incidents.');
        }
        
        // Get staff for assignment dropdown (only for roles that can assign)
        $staff = [];
        if ($user->hasAnyRole(['HR Officer', 'HOD', 'System Admin'])) {
            $staff = User::where('is_active', true)
                ->whereHas('roles', function($q) {
    ...
    public function store(Request $request)
    {
        ...
        if (!$user->hasAnyRole(['HR Officer', 'HOD', 'System Admin', 'Reception Person'])) {
            return response()->json([
                'success' => false,
                'message' => 'Unauthorized. You do not have permission to create incidents.'
            ], 403);
        }
        ...
        try {
            ...
            // Determine assignment - auto-assign to branch manager for Reception Person if not explicitly assigned
            $assignedTo = $request->assigned_to;
            $assignedBy = $request->assigned_to ? $user->id : null;
            $assignedAt = $request->assigned_to ? now() : null;
            $status = $request->assigned_to ? 'Assigned' : 'New';
            
            // Auto-assign to branch manager if created by Reception Person and no explicit assignment
            if ($user->hasRole('Reception Person') && !$request->assigned_to && $user->branch) {
                $branchManager = $user->branch->managers()->where('is_active', true)->first();
                if ($branchManager) {
                    $assignedTo = $branchManager->id;
                    $assignedBy = $user->id;
                    $assignedAt = now();
                    $status = 'Assigned';
                }
