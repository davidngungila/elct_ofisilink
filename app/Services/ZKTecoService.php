    ...
    public function syncAttendancesToDatabase(): array
    {
        try {
            ...
            foreach ($attendances as $index => $log) {
                ...
                // Get attendance date as Carbon instance and string
                $attendanceDate = $punchTime->copy()->startOfDay();
                ...
                $result = DB::transaction(function() use ($user, $enrollId, $attendanceDate, $attendanceDateStr, $punchTime, $verifyMode) {
                    ...
                    $attendance = \App\Models\Attendance::where('user_id', $user->id)
                        ->where(function($query) use ($attendanceDateStr) {
                            $query->where('attendance_date', $attendanceDateStr)
                                  ->orWhere(function($q) use ($attendanceDateStr) {
                                      // For old records without attendance_date, check by punch_time date
                                      $q->whereNull('attendance_date')
                                        ->whereDate('punch_time', $attendanceDateStr);
                                  });
                        })
                    ...
                    if (!$attendance) {
                        ...
                        if (!$attendance->attendance_date) {
                            $attendance->attendance_date = $attendanceDateStr;
                            $attendance->save();
                        }
                    }
                    ...
                    if (!$hasCheckIn) {
                        ...
                        $attendance->update([
                            ...
                            'status' => Attendance::STATUS_PRESENT, // Set to PRESENT when checked in
                            'verify_mode' => $verifyMode,
                            'verification_status' => Attendance::VERIFICATION_VERIFIED, // Auto-verify device records
                            'attendance_date' => $attendanceDateStr, // Ensure date is set
                        ]);
                        ...
                        try {
                            ...
                            Log::warning('Failed to send check-in SMS', [
                                'user_id' => $user->id,
                        ...
                        // Keep status as PRESENT (not absent) when checked out
                        $updateResult = $attendance->update([
                            'check_out_time' => $checkOutTime,
                            'punch_time' => $checkOutTime, // Update punch_time to latest
                            'status' => Attendance::STATUS_PRESENT, // Keep as PRESENT when checked out
                            'verify_mode' => $verifyMode,
                            'verification_status' => Attendance::VERIFICATION_VERIFIED, // Auto-verify device records
                            'attendance_date' => $attendanceDateStr, // Ensure date is set
                        ...
                        if ($attendance->check_in_time && $attendance->check_out_time) {
                            try {
                                ...
                                // Check for early leave and overtime (if schedule exists)
                                try {
                                    if ($attendance->schedule_id) {
                                        $schedule = \App\Models\WorkSchedule::find($attendance->schedule_id);
                                        if ($schedule && $schedule->end_time) {
                                            $expectedEnd = Carbon::parse($attendanceDate->format('Y-m-d') . ' ' . $schedule->end_time->format('H:i:s'));
                                            ...
                                            // Check for overtime
                                            $overtimeThreshold = $schedule->overtime_threshold_minutes ?? 30;
                                            if ($checkOut->gt($expectedEnd->copy()->addMinutes($overtimeThreshold))) {
                                                $attendance->is_overtime = true;
                                                // Keep as PRESENT but mark overtime flag
                                            }
                                        }
                                    }
                                } catch (\Exception $e) {
                                    Log::warning('Error checking early leave/overtime', ['error' => $e->getMessage()]);
                                }
                                
                                $attendance->save();
                            } catch (\Exception $e) {
                                Log::warning('Error calculating hours for check-out', ['error' => $e->getMessage()]);
                            }
                        }
                        ...
                        try {
                            ...
                            if ($employeePhone) {
                                ...
                                $timeOut = $checkOutTime->format('h:i A');
                                $date = $attendanceDate->format('M d, Y');
                                ...
                                $hour = (int)$checkOutTime->format('H');
                                $inspirationalMessage = '';
                                if ($hour >= 17 && $hour < 22) {
                                    $eveningMessages = [
                            ...
                            Log::warning('Failed to send check-out SMS', [
                                'user_id' => $user->id,
                                'error' => $smsError->getMessage()
                            ]);
                        }
