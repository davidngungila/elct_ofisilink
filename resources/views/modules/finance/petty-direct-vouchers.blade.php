...
<div class="card shadow-sm border-warning">
  ...
  <div class="card-body">
      ...
      <div class="card-header bg-gradient-light border-bottom">
        <div class="d-flex justify-content-between align-items-center">
          ...
          <button class="btn btn-sm btn-link text-decoration-none" type="button" data-bs-toggle="collapse" data-bs-target="#filterCollapse" aria-expanded="true">
            <i class="bx bx-chevron-up" id="filterToggleIcon"></i>
          </button>
        </div>
      </div>
      <div class="collapse show" id="filterCollapse">
        <div class="card-body p-3">
          <form method="GET" action="{{ route('petty-cash.direct-vouchers.index') }}" id="filterForm">
            <input type="hidden" name="status" value="{{ $status }}">
            <div class="row g-3">
              <div class="col-md-4">
                <label class="form-label small fw-bold mb-1">
                  <i class="bx bx-search me-1"></i>Quick Search
                </label>
              ...
              <div class="col-md-2">
                <label class="form-label small fw-bold mb-1">
                  <i class="bx bx-calendar me-1"></i>Date To
                </label>
              ...
              <div class="col-md-2">
                <label class="form-label small fw-bold mb-1">
                  <i class="bx bx-money me-1"></i>Min Amount
                </label>
                <input type="number" class="form-control form-control-sm" name="amount_min" 
                       value="{{ request('amount_min') }}" placeholder="0.00" step="0.01" min="0">
              </div>
              <div class="col-md-2">
                <label class="form-label small fw-bold mb-1">
                  <i class="bx bx-money me-1"></i>Max Amount
                </label>
                <input type="number" class="form-control form-control-sm" name="amount_max" 
                       value="{{ request('amount_max') }}" placeholder="0.00" step="0.01" min="0">
              </div>
            </div>
            <div class="row mt-2">
              <div class="col-12 d-flex gap-2">
                ...
                <span class="badge bg-info align-self-center ms-auto">
                  <i class="bx bx-info-circle me-1"></i>{{ $vouchers->total() }} result(s)
                </span>
                @endif
              </div>
            </div>
    ...
    <div class="table-responsive">
      <table class="table table-hover align-middle mb-0" id="directVouchersTable">
        <thead class="table-light">
          <tr>
            <th class="fw-bold" style="width: 140px;">
              <i class="bx bx-hash me-1"></i>Voucher No
            </th>
            <th class="fw-bold" style="min-width: 160px;">
              <i class="bx bx-user me-1"></i>Created By
            </th>
            <th class="fw-bold" style="width: 130px;">
              <i class="bx bx-calendar me-1"></i>Date
            </th>
            <th class="fw-bold text-end" style="width: 150px;">
              <i class="bx bx-money me-1"></i>Amount (TZS)
            </th>
            <th class="fw-bold" style="min-width: 220px;">
              <i class="bx bx-file-blank me-1"></i>Purpose
            </th>
            <th class="fw-bold text-center" style="width: 130px;">
              <i class="bx bx-info-circle me-1"></i>Status
            </th>
            <th class="fw-bold text-center" style="width: 180px;">
          ...
          <tr class="voucher-row" data-voucher-id="{{ $voucher->id }}">
            <td>
              <div class="d-flex flex-column gap-1">
                <span class="badge bg-primary rounded-pill px-3 py-1">
                  <i class="bx bx-hash me-1"></i>{{ $voucher->voucher_no }}
                </span>
                <span class="badge bg-warning text-dark rounded-pill px-2 py-1" style="font-size: 0.7rem;" title="Direct Voucher (In-Office)">
            ...
            <td>
              <div class="d-flex align-items-center">
                ...
                <div class="flex-grow-1">
                  <div class="fw-semibold">{{ $voucher->creator->name ?? 'N/A' }}</div>
                  @if($voucher->creator && $voucher->creator->employee_id)
                  <small class="text-muted">
                    <i class="bx bx-id-card me-1"></i>{{ $voucher->creator->employee_id }}
                  </small>
                  @endif
                </div>
              </div>
            ...
            <td>
              <div class="d-flex flex-column">
                <span class="fw-semibold">{{ $voucher->date->format('M d, Y') }}</span>
                <small class="text-muted">{{ $voucher->date->format('D') }}</small>
              </div>
            </td>
            <td class="text-end">
              <div class="d-flex flex-column align-items-end">
                <strong class="text-success fs-6">TZS {{ number_format($voucher->amount, 2) }}</strong>
                @if($voucher->lines && $voucher->lines->count() > 0)
                <small class="text-muted">{{ $voucher->lines->count() }} line(s)</small>
                @endif
              </div>
            </td>
            <td>
              <div class="d-flex align-items-start">
                <i class="bx bx-file-blank text-muted me-2 mt-1"></i>
                <div>
                  <div class="text-truncate" style="max-width: 200px;" title="{{ $voucher->purpose }}">
                    {{ Str::limit($voucher->purpose, 60) }}
                  </div>
                  @if($voucher->payee)
                  <small class="text-muted">
                    <i class="bx bx-user me-1"></i>{{ Str::limit($voucher->payee, 30) }}
                  </small>
                  @endif
                </div>
              </div>
            ...
            <td class="text-center">
              ...
              <span class="badge bg-warning bg-opacity-20 text-warning border border-warning rounded-pill px-3 py-2">
                <i class="bx bx-time-five me-1"></i>Pending HOD
              </span>
              ...
              <span class="badge bg-success bg-opacity-20 text-success border border-success rounded-pill px-3 py-2">
                <i class="bx bx-check-circle me-1"></i>Paid (Complete)
              </span>
              @else
              <span class="badge bg-danger bg-opacity-20 text-danger border border-danger rounded-pill px-3 py-2">
                <i class="bx bx-x-circle me-1"></i>{{ ucwords(str_replace('_', ' ', $voucher->status)) }}
              </span>
              @endif
            </td>
            <td>
              <div class="d-flex gap-1 justify-content-center">
                <a href="{{ route('petty-cash.show', $voucher) }}" 
                   ...
                   title="View Full Details" data-bs-toggle="tooltip">
                ...
                <button type="button" class="btn btn-sm btn-outline-success rounded-pill px-3" 
                        ...
                        data-bs-target="#approveModal{{ $voucher->id }}" 
                        title="Approve & Mark as Paid" data-bs-toggle="tooltip">
                ...
                <button type="button" class="btn btn-sm btn-outline-danger rounded-pill px-3" 
                        ...
                        title="Reject Voucher" data-bs-toggle="tooltip">
                  <i class="bx bx-x"></i>
    ...
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-center mt-4 pt-3 border-top">
      <div class="mb-2 mb-md-0">
        <small class="text-muted">
          ...
          Showing <strong>{{ $vouchers->firstItem() ?? 0 }}</strong> to <strong>{{ $vouchers->lastItem() ?? 0 }}</strong> 
          of <strong>{{ $vouchers->total() }}</strong> direct vouchers
        </small>
      </div>
      <div>
        {{ $vouchers->links() }}
    ...
    <div class="text-center py-5">
      <div class="mb-4">
        <i class="bx bx-file-blank" style="font-size: 5rem; color: #dee2e6;"></i>
      </div>
      <h5 class="text-muted mb-2">No Direct Vouchers Found</h5>
      <p class="text-muted mb-4">
        @if($status == 'pending_hod')
        There are no direct vouchers pending HOD approval at this time.
        @elseif($status == 'paid')
        No paid direct vouchers found.
        @elseif($status == 'rejected')
        No rejected direct vouchers found.
        @else
        No direct vouchers found for this status.
        @endif
      </p>
      @if($isAccountant && $status != 'pending_hod')
      <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#directVoucherModal">
        <i class="bx bx-plus-circle me-2"></i>Create New Direct Voucher
      </button>
      @endif
    </div>
    @endif
