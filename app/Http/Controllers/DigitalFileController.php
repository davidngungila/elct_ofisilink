...
class DigitalFileController extends Controller
{
    ...
    private function handleCreateFolder(Request $request)
    {
        ...
        if ($request->parent_id && $request->parent_id != 0) {
            $parentFolder = FileFolder::find($request->parent_id);
            $branchId = $parentFolder->branch_id ?? $user->branch_id;
        } else {
    ...
    private function handleUploadFile(Request $request)
    {
        ...
        $branchId = $folder->branch_id ?? $user->branch_id;
    ...
    private function handleGetDashboardStats(Request $request)
    {
        $user = Auth::user();
        $userRoles = $user->roles()->pluck('name')->toArray();
        $canViewAll = in_array('System Admin', $userRoles) || in_array('General Manager', $userRoles) || in_array('HR Officer', $userRoles) || in_array('Record Officer', $userRoles);
        $currentDeptId = $user->department_id ?? null;
    ...
    private function getDashboardStats($user, $canViewAll, $currentDeptId, $branchId = null)
    {
        ...
        if ($branchId) {
            $filesQuery->where('branch_id', $branchId);
            $foldersQuery->where('branch_id', $branchId);
        } elseif (!$canViewAll && $user->branch_id) {
            // Auto-filter by user's branch if not admin
            $filesQuery->where('branch_id', $user->branch_id);
            $foldersQuery->where('branch_id', $user->branch_id);
        }
        
        // Always get actual total counts for selected branch
        $totalFiles = $filesQuery->count();
        $totalFolders = $foldersQuery->count();
        
        // For user-specific stats, apply filters
        $userQuery = clone $filesQuery;
        if (!$canViewAll) {
        ...
        $publicFolders = (clone $foldersQuery)->where('access_level', 'public')->count();
    ...
    public function dashboard(Request $request)
    {
        ...
        // Get selected branch from request or use user's branch
        $selectedBranchId = $request->input('branch_id', $userBranchId);
        
        // Get all branches for dropdown
        $branches = \App\Models\Branch::where('is_active', true)->orderBy('name')->get();
        
        // Get all folders (hierarchical) - filter by branch
        $foldersQuery = FileFolder::with(['creator', 'department', 'parent', 'branch'])
            ->withCount(['files', 'subfolders']);
        
        // Filter by branch if not System Admin or General Manager viewing all
        if ($selectedBranchId && !$canViewAll) {
            $foldersQuery->where('branch_id', $selectedBranchId);
        } elseif ($selectedBranchId && $canViewAll) {
            // Allow filtering by branch even for admins
            $foldersQuery->where('branch_id', $selectedBranchId);
        ...
        $stats = $this->getDashboardStats($user, $canViewAll, $currentDeptId, $selectedBranchId);
        
        // Get recent activity (filtered by branch)
        $recentActivityQuery = FileActivity::with(['user', 'file.folder'])
            ->whereHas('file', function($q) use ($selectedBranchId, $userBranchId, $canViewAll) {
                if ($selectedBranchId) {
    ...
    public function manage()
    {
        ...
        // Get files filtered by branch
        $filesQuery = FileModel::with(['uploader', 'folder', 'branch'])
            ->orderBy('created_at', 'desc');
        
        // Filter by branch
        if ($selectedBranchId && !$canViewAll) {
            $foldersQuery->where('branch_id', $selectedBranchId);
            $filesQuery->where('branch_id', $selectedBranchId);
        } elseif ($selectedBranchId && $canViewAll) {
            $foldersQuery->where('branch_id', $selectedBranchId);
            $filesQuery->where('branch_id', $selectedBranchId);
        } elseif (!$canViewAll && $userBranchId) {
            $foldersQuery->where('branch_id', $userBranchId);
            $filesQuery->where('branch_id', $userBranchId);
        }
    ...
    public function search()
    {
        ...
        // Get folders filtered by branch
        $foldersQuery = FileFolder::with('branch')->orderBy('name');
        ...
        return view('modules.files.digital.search', compact('folders', 'departments', 'canViewAll', 'currentDeptId', 'user', 'userBranchId'));
    }
    ...
    public function analytics()
    {
        ...
        // Get comprehensive statistics (filtered by branch)
        $stats = $this->getDashboardStats($user, $canViewAll, $currentDeptId, $selectedBranchId);
        
        // Build base query with access control and branch filtering
        $filesQuery = FileModel::query();
        ...
        $departmentStatsQuery = FileModel::join('file_folders', 'files.folder_id', '=', 'file_folders.id')
            ->leftJoin('departments', 'file_folders.department_id', '=', 'departments.id');
        
        if ($selectedBranchId && !$canViewAll) {
            $departmentStatsQuery->where('files.branch_id', $selectedBranchId);
        } elseif ($selectedBranchId && $canViewAll) {
            $departmentStatsQuery->where('files.branch_id', $selectedBranchId);
        } elseif (!$canViewAll && $userBranchId) {
            $departmentStatsQuery->where('files.branch_id', $userBranchId);
        }
        
        $departmentStats = $departmentStatsQuery->select(
        ...
        // Most active users (by uploads) - filtered by branch
        $topUploadersQuery = FileModel::join('users', 'files.uploaded_by', '=', 'users.id');
        
        if ($selectedBranchId && !$canViewAll) {
            $topUploadersQuery->where('files.branch_id', $selectedBranchId);
        } elseif ($selectedBranchId && $canViewAll) {
            $topUploadersQuery->where('files.branch_id', $selectedBranchId);
        } elseif (!$canViewAll && $userBranchId) {
            $topUploadersQuery->where('files.branch_id', $userBranchId);
        }
        
        $topUploaders = $topUploadersQuery->select(
                'users.id',
                'users.name',
        ...
        // Most downloaded files (filtered by branch)
        $topDownloadedQuery = FileModel::query();
        if ($selectedBranchId && !$canViewAll) {
            $topDownloadedQuery->where('branch_id', $selectedBranchId);
        } elseif ($selectedBranchId && $canViewAll) {
            $topDownloadedQuery->where('branch_id', $selectedBranchId);
        } elseif (!$canViewAll && $userBranchId) {
            $topDownloadedQuery->where('branch_id', $userBranchId);
        }
        
        $topDownloaded = $topDownloadedQuery->select(
                'id',
                'original_name',
                'download_count',
                'file_size',
        ...
        // Recent uploads (last 7 days) - filtered by branch
        $recentUploadsQuery = FileModel::with(['uploader', 'folder']);
        if ($selectedBranchId && !$canViewAll) {
            $recentUploadsQuery->where('branch_id', $selectedBranchId);
        } elseif ($selectedBranchId && $canViewAll) {
            $recentUploadsQuery->where('branch_id', $selectedBranchId);
        } elseif (!$canViewAll && $userBranchId) {
            $recentUploadsQuery->where('branch_id', $userBranchId);
        }
        
        $recentUploads = $recentUploadsQuery->where('created_at', '>=', now()->subDays(7))
            ->orderBy('created_at', 'desc')
            ->limit(10)
            ->get();
        
        // Activity type breakdown (filtered by branch)
        $activityTypesQuery = FileActivity::whereHas('file', function($q) use ($selectedBranchId, $userBranchId, $canViewAll) {
            if ($selectedBranchId && !$canViewAll) {
                $q->where('branch_id', $selectedBranchId);
            } elseif ($selectedBranchId && $canViewAll) {
                $q->where('branch_id', $selectedBranchId);
        ...
        $activityTypes = $activityTypesQuery->select(
                'activity_type',
                DB::raw('count(*) as count')
            )
        ...
        // Activity over time (last 30 days) - detailed (filtered by branch)
        $activityDataQuery = FileActivity::whereHas('file', function($q) use ($selectedBranchId, $userBranchId, $canViewAll) {
            if ($selectedBranchId && !$canViewAll) {
                $q->where('branch_id', $selectedBranchId);
            } elseif ($selectedBranchId && $canViewAll) {
                $q->where('branch_id', $selectedBranchId);
        ...
        $activityData = $activityDataQuery->select(
        ...
        // Weekly activity summary (filtered by branch)
        $weeklyActivityQuery = FileActivity::whereHas('file', function($q) use ($selectedBranchId, $userBranchId, $canViewAll) {
            if ($selectedBranchId && !$canViewAll) {
                $q->where('branch_id', $selectedBranchId);
            } elseif ($selectedBranchId && $canViewAll) {
                $q->where('branch_id', $selectedBranchId);
        ...
        $weeklyActivity = $weeklyActivityQuery->select(
        ...
        // Folder statistics with size (filtered by branch)
        $folderStatsQuery = FileFolder::withCount(['files'])
            ->select('file_folders.*')
            ->selectRaw('COALESCE(SUM(files.file_size), 0) as total_size')
            ->leftJoin('files', 'file_folders.id', '=', 'files.folder_id');
        
        if ($selectedBranchId && !$canViewAll) {
            $folderStatsQuery->where('file_folders.branch_id', $selectedBranchId);
        } elseif ($selectedBranchId && $canViewAll) {
            $folderStatsQuery->where('file_folders.branch_id', $selectedBranchId);
        } elseif (!$canViewAll && $userBranchId) {
            $folderStatsQuery->where('file_folders.branch_id', $userBranchId);
        }
        
        $folderStats = $folderStatsQuery->groupBy('file_folders.id', 'file_folders.name', 'file_folders.created_at', 'file_folders.updated_at', 'file_folders.parent_id', 'file_folders.path', 'file_folders.access_level', 'file_folders.department_id', 'file_folders.created_by', 'file_folders.folder_code', 'file_folders.description', 'file_folders.branch_id')
            ->orderBy('total_size', 'desc')
            ->limit(20)
            ->get();
        
        // Access request statistics (filtered by branch)
        $accessRequestStatsQuery = FileAccessRequest::whereHas('file', function($q) use ($selectedBranchId, $userBranchId, $canViewAll) {
            if ($selectedBranchId && !$canViewAll) {
                $q->where('branch_id', $selectedBranchId);
            } elseif ($selectedBranchId && $canViewAll) {
                $q->where('branch_id', $selectedBranchId);
        ...
        $accessRequestStats = $accessRequestStatsQuery->select(
                'status',
                DB::raw('count(*) as count')
            )
            ->groupBy('status')
        ...
        // Assignment statistics (filtered by branch)
        $assignmentStatsQuery = FileUserAssignment::where(function($q) use ($selectedBranchId, $userBranchId, $canViewAll) {
            $q->whereHas('file', function($fileQ) use ($selectedBranchId, $userBranchId, $canViewAll) {
                if ($selectedBranchId && !$canViewAll) {
                    $fileQ->where('branch_id', $selectedBranchId);
                } elseif ($selectedBranchId && $canViewAll) {
                    $fileQ->where('branch_id', $selectedBranchId);
                } elseif (!$canViewAll && $userBranchId) {
                    $fileQ->where('branch_id', $userBranchId);
                }
            })->orWhereHas('folder', function($folderQ) use ($selectedBranchId, $userBranchId, $canViewAll) {
                if ($selectedBranchId && !$canViewAll) {
                    $folderQ->where('branch_id', $selectedBranchId);
                } elseif ($selectedBranchId && $canViewAll) {
                    $folderQ->where('branch_id', $selectedBranchId);
                } elseif (!$canViewAll && $userBranchId) {
                    $folderQ->where('branch_id', $userBranchId);
                }
            });
        });
        
        $assignmentStats = $assignmentStatsQuery->select(
        ...
        // Storage growth over time (monthly) - filtered by branch
        $storageGrowthQuery = FileModel::query();
        if ($selectedBranchId && !$canViewAll) {
            $storageGrowthQuery->where('branch_id', $selectedBranchId);
        } elseif ($selectedBranchId && $canViewAll) {
            $storageGrowthQuery->where('branch_id', $selectedBranchId);
        } elseif (!$canViewAll && $userBranchId) {
            $storageGrowthQuery->where('branch_id', $userBranchId);
        }
        
        $storageGrowth = $storageGrowthQuery->select(
    ...
    public function accessRequests()
    {
        ...
        // Get pending requests filtered by branch
        $pendingRequestsQuery = FileAccessRequest::with(['user', 'file.folder', 'file.branch'])
            ->where('status', 'pending')
            ->whereHas('file', function($q) use ($selectedBranchId, $userBranchId, $canViewAll) {
                if ($selectedBranchId && !$canViewAll) {
                    $q->where('branch_id', $selectedBranchId);
        ...
        // Get user's own requests (always show user's own requests regardless of branch)
        $myRequests = FileAccessRequest::with(['user', 'file.folder', 'file.branch'])
            ->where('user_id', $user->id)
            ->orderBy('created_at', 'desc')
            ->get();
    ...
    public function activityLog()
    {
        ...
        // Get activities filtered by branch
        $activitiesQuery = FileActivity::with(['user', 'file.folder', 'file.branch'])
            ->whereHas('file', function($q) use ($selectedBranchId, $userBranchId, $canViewAll) {
                if ($selectedBranchId && !$canViewAll) {
                    $q->where('branch_id', $selectedBranchId);
                } elseif ($selectedBranchId && $canViewAll) {
    ...
    public function settings()
    {
        ...
        // Get storage statistics filtered by branch
        $storageQuery = FileModel::query();
        if ($selectedBranchId && !$canViewAll) {
            $storageQuery->where('branch_id', $selectedBranchId);
        } elseif ($selectedBranchId && $canViewAll) {
            $storageQuery->where('branch_id', $selectedBranchId);
        } elseif (!$canViewAll && $userBranchId) {
            $storageQuery->where('branch_id', $userBranchId);
        }
        
        $totalStorage = $storageQuery->sum('file_size');
