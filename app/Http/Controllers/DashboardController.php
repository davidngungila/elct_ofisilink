    ...
    public function adminDashboard()
    {
        ...
        $pendingApprovals = [
            ...
            'meeting_minutes' => DB::table('meeting_minutes')
                ...
                ->where('meeting_minutes.status', 'pending_approval')
                ->where('meeting_minutes.approver_id', $user->id)
                ->select('meeting_minutes.*', 'meetings.title as meeting_title', 'meetings.meeting_date', 'users.name as approver_name')
                ->orderBy('meeting_minutes.created_at', 'desc')
                ->limit(5)
                ->get(),
        ];
    ...
    public function hodDashboard()
    {
        ...
        $pendingApprovals = [
            'meeting_minutes' => DB::table('meeting_minutes')
                ...
                ->where('meeting_minutes.status', 'pending_approval')
                ->where('meeting_minutes.approver_id', $user->id)
                ->select('meeting_minutes.*', 'meetings.title as meeting_title', 'meetings.meeting_date')
                ->orderBy('meeting_minutes.created_at', 'desc')
                ->limit(10)
                ->get(),
